...
import { PUPPET_EXTENSION_RDNS, WALLET_EVENT, WALLET_TARGET } from '@puppet/sdk/wallet'
...
const PROVIDER_INFO: EIP6963ProviderInfo = {
  // EIP-6963 requires a fresh UUIDv4 per provider instance (one per page load).
...
export default defineUnlistedScript(() => {
  ...
  // The single place connection state advances: dedupes, fires `connect` on the first
  // account, and mirrors every transition (including disconnect, accounts = []) to listeners.
  const applyAccounts = (accounts: string[]): void => {
    const next = accounts[0] ?? null
    if (next === connectedAddress) return
    const wasConnected = connectedAddress !== null
    connectedAddress = next
    if (next && !wasConnected) emit('connect', { chainId: HUB_CHAIN_ID_HEX })
    emit(WALLET_EVENT.ACCOUNTS_CHANGED, accounts)
  }

  // Handle RPC responses and background events
  window.addEventListener('message', event => {
    if (event.source !== window || event.data?.target !== WALLET_TARGET.INPAGE) return
    ...
    if (id !== undefined && pending.has(id)) {
      ...
      pending.delete(id)
      if (response?.error) {
        // Reject with an EIP-1193 ProviderRpcError (numeric code) so dApp libraries (ethers
        // v6 coalesceError, wagmi) surface our actual message instead of "Unknown connector
        // error". Default to -32603 (internal); the user-rejection path supplies 4001.
        const err = new Error(response.error.message ?? 'Puppet request failed') as Error & {
          code: number
          data?: unknown
        }
        err.code = typeof response.error.code === 'number' ? response.error.code : -32603
    ...
    // Background event (e.g. accountsChanged after a connect / disconnect).
    if (payload?.type === 'event') {
      const { event: eventName, args } = payload
      if (eventName === WALLET_EVENT.ACCOUNTS_CHANGED) {
        const accounts = args[0] as string[]
        // A connect can outlive its original message channel (the MV3 worker recycles
        // while eth_requestAccounts waits for the site handshake): the broadcast is the
        // durable signal, so resolve any pending account request from it directly.
        if (accounts.length > 0) {
          for (const [pendingId, entry] of pending) {
            if (entry.method !== 'eth_requestAccounts' && entry.method !== 'eth_accounts') continue
        ...
        pending.set(id, {
          ...
          resolve: (result: unknown) => {
            if (method === 'eth_requestAccounts' || method === 'eth_accounts') {
              applyAccounts(result as string[])
            }
            resolve(result)
          },
          reject
        })
        window.postMessage({ target: WALLET_TARGET.CONTENT, id, payload: { method, params } }, '*')
      })
    },
    on(event, listener) {
