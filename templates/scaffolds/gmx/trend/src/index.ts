...
async function tick(): Promise<void> {
  ...
  // Recognize base GMX returned from a prior close so it's spendable again (otherwise
  // signedBalance ratchets down and the next open reverts).
  ...
  if (wantLong && !inLong) {
    ...
    await gmx.createOrder({
      ...
      collateralDelta: 0n, // GMX returns the position's collateral on a full close
      acceptablePrice: gmxPrice(price * (1 - SLIPPAGE_BPS / 10_000), 18)
    })
    lastTradeAt = Date.now()
  }
