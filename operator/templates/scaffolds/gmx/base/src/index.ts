...
// holds capital and carries positions), lifecycle, and the generic operate() escape hatch.
...
// session key (in memory) and the site's matchmaker/indexer endpoints. RPC defaults to the
// public Arbitrum endpoint — set ARBITRUM_RPC_URL for anything beyond a first run.
// Everything below is an optional .env override (point at a local site, use your own
// endpoints, or run headless). The operator can open and manage positions within the rules
// you signed, but can never withdraw.
...
await runOperator(core, async signal => {
  ...
  //          returns here on its own) · isOpen() · status · operate(callList)
