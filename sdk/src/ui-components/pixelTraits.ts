export type IPixelArt = { x?: number; y?: number; palette: string[]; rows: string[]; weight?: number }
export type IGradientArt = { gradient: [string, string]; weight?: number }
export type ITraitArt = IPixelArt | IGradientArt

export type IPixelCategory = { name: string; traits: Record<string, ITraitArt> }
...
const headBlock = (skin: string): IPixelArt => ({
  x: 4,
  y: 4,
...
const headBands = (colorList: string[]): IPixelArt => {
...
const background: IPixelCategory = {
  name: 'background',
  traits: {
...
export const PIXEL_LAYERS: IPixelCategory[] = [background, head, neck, mouth, eyes, face, headwear]
