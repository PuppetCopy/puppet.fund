...
import { type IGradientArt, INK, type IPixelCategory, type ITraitArt, PIXEL_LAYERS } from './pixelTraits.js'
...
const weightedPick = (seed: string, category: IPixelCategory) => {
...
export const pixelTraitsFromAddress = (address: Address): Record<string, string> => {
  const seed = address.toLowerCase()
  const traits: Record<string, string> = {}
  for (const category of PIXEL_LAYERS) traits[category.name] = weightedPick(seed, category)
  return traits
}
...
export const pixelAvatarName = (address: Address): string => {
  const traits = pixelTraitsFromAddress(address)
...
export const pixelAvatarSvg = (address: Address): string => {
  const seed = address.toLowerCase()
  const idSuffix = seed.slice(2, 8)
  const chosen = pixelTraitsFromAddress(address)

  const headArt = PIXEL_LAYERS[1].traits[chosen.head]
  ...
  let defs = ''
  let body = ''
  for (const category of PIXEL_LAYERS) {
