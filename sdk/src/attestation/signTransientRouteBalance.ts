...
export interface ISignTransientRouteBalanceInput {
...
export interface ISignTransientRouteBalanceAttestContext extends IDraftContext {
  currentBlock: bigint
  signedBalance: bigint
  transientRouteBalance: bigint
}

export function attestSignTransientRouteBalanceIntent(
  ctx: ISignTransientRouteBalanceAttestContext,
  ...
  if (ctx.transientRouteBalance < input.amount) {
    throw new CompactContractError('Deposit__InsufficientBalance', [ctx.transientRouteBalance, input.amount])
  }
  IntentLib.verifyRelayFee(input.acceptableRelayFee, ctx.signedBalance + input.amount)

  const intent: ICoreGate__SignTransientRouteBalanceIntent = {
    params: input.params,
  ...
  const typedData: TypedDataDefinition = {
    domain,
    ...CORE_GATE_INTENTS.signTransientRouteBalance,
