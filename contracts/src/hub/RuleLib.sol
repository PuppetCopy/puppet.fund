...
library RuleLib {
    bytes32 internal constant SUBSCRIBE_RULE_TYPEHASH = keccak256(
        "SubscribeRule(AccountInitParams masterParams,bytes body,bytes mandate)AccountInitParams(address user,bytes32 name,bytes32 baseTokenId,address signer)"
    );
    ...
    function hashRule(
        ...
        return keccak256(
            abi.encode(
                SUBSCRIBE_RULE_TYPEHASH,
                AccountLib.hashAccount(_rule.masterParams),
                keccak256(_rule.body),
                keccak256(_rule.mandate)
            )
        );
    }
    ...
    function mandate(
        ...
        digest_ = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encode(MANDATE_TYPEHASH, _puppet, _master, address(_baseToken), bodyHash_))
            )
        );
    }
