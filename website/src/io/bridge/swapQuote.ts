...
// One generic swap: input token on a SPOKE chain to the output token at the route on the
// hub. The bridge intent is structurally cross-chain (Deposit__SameChainBridge), so
// same-chain swaps are not offerable. Eligible providers are quoted in parallel and the
// best output wins; the on-chain side is provider-agnostic (PassthroughRoute executes
// whatever provider call the winning quote encodes).
export async function fetchSwapQuote(p: {
  ...
  if (p.originChainId === p.destinationChainId) {
    throw new Error('Same-chain swaps are not supported: the bridge intent is cross-chain only')
  }
